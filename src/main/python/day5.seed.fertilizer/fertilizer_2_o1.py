...
def find_location(text):
    extracted_seeds = list(map(int, extract_seeds(text)))
    ...
    for map in maps:
        print(map)
        # while i+1 < len(extracted_seeds):
        s = find_lowest_location(extracted_seeds[i], extracted_seeds[i + 1], map)
...
def extract_intervals(seeds, map):
    source_range = map["source_range"]
    length = map["length"]
    dest_range = map["dest_range"]

def divide_intervals(seed, rang, map):


def find_lowest_location(seed, seed_range, list_of_maps):
    if not list_of_maps:
        return seed
...
def find_lowest_loc(seed, seed_range, map):
    in_range = {}
